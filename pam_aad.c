...
STATIC int verify_user(jwt_t *jwt, const char *username)
{
    ...
    if (!upn)
    {
        upn = jwt_get_grant(jwt, "unique_name");
    }
...
STATIC int azure_authenticator(const char *user)
{
    ...
    if (json_object_get(config, "debug"))
        if (strcmp(json_string_value(json_object_get(config, "debug")),
                   "true") == 0)
            debug = true;
    ...
    {
        client_id =
            json_string_value(json_object_get(json_object_get(config, "client"), "id"));
    }
    else
    {
    ...
    */
    ...
    {
        tenant =
            json_string_value(json_object_get(json_object_get(config, "tenant"), "name"));
        if (json_object_get(json_object_get(config, "tenant"), "address"))
        {
            tenant_addr =
                json_string_value(json_object_get(json_object_get(config, "tenant"),
                                                  "address"));
        }
        else
        {
    ...
    */
    ...
    if (verify_user(jwt, user_addr) == 0 /* && verify_group(ab_token, group_id, debug) == 0 */)
    {
        ret = EXIT_SUCCESS;
    }
